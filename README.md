...
and stitches the clips with FFmpeg into a crossfaded, exactly-timed, downloadable 1080p MP4.
...
Each shot's still is animated into a five-second clip through the Higgsfield Director-of-Photography
image-to-video model. The engine uploads the normalized photo to the platform CDN, submits the
prompt with the platform prompt-enhancer disabled, polls the request to completion, and downloads
the finished clip. It runs two clips concurrently, isolates per-shot failures so a single bad clip
is skipped rather than fatal, and splits its retry logic so that transient pre-submit errors are
retried while a billed job is never resubmitted.

### Cinematic render

FFmpeg normalizes every clip to a common frame, constant frame rate and timebase, trims each to
its exact storyboard duration, and chains crossfade transitions with offsets computed so the
overall cut length is precise. Output is H.264 1080p at CRF 19 with a faststart flag for instant
web playback. Failed shots are simply omitted, so a five-of-six tour still renders.
...
```
                   ...
                   |                                              |
  Upload  ->  Vision  ->  Storyboard  ->  Prompt  ->  VideoGen  ->  Render  ->  MP4
  (sharp)    (Claude)     (rule-based)    (template)  (Higgsfield)  (FFmpeg)
...
5. **VideoGen.** For each shot: request a presigned upload URL, PUT the photo bytes to the platform
   CDN, submit the returned public URL with the prompt and duration and `enhance_prompt: false`, poll
   the request until it completes, and download the clip. Two shots animate concurrently. Obtaining a
   request id is retried on transient errors; polling and download are never retried, so a slow clip
   is never resubmitted and never double-billed.

6. **Render.** FFmpeg builds one `filter_complex`: normalize and trim every clip, then chain
   crossfades where transition `k` begins at `sum(durations[0..k-1]) - k * crossfade`. The graph is
   encoded to H.264 1080p, and encode progress is parsed from FFmpeg's stderr time output.
...
translational move aimed at a prompt-named target will fabricate that target. The system reduces this
on the generative path with two levers: the prompt carries no scene description and no directional
target, only a neutral move and a hard "do not add, remove, move, or invent anything" constraint;
and the platform prompt-enhancer, which embellishes and invents detail, is disabled on submit. These
reduce drift substantially. They cannot eliminate it, because a generative model always retains
latitude and the endpoint has no negative-prompt field, so the fidelity constraint is documented as a
...
| `POST` | `/api/runs` | Start a run. `multipart/form-data` with real photos, or `application/json` for the built-in demo set. Optional `review` flag pauses before clip spend. |
...
| `GET` | `/api/projects/:id/video` | Stream the finished MP4 with range support; `?download` forces an attachment. |
...
| Claude vision | Room understanding via structured JSON outputs. |
| Higgsfield image-to-video | Generative clip animation (Director-of-Photography model). |
| `sharp` | EXIF rotate, normalize, resize, thumbnails. |
| `ffmpeg-static` | Per-clip normalize, trim, crossfade, H.264 encode. |
...
packages/
  ...
  engine-videogen/   Higgsfield upload, submit, poll, download
  engine-render/     FFmpeg normalize, trim, crossfade, encode
...
`ANTHROPIC_API_KEY` for real Claude analysis and `HIGGSFIELD_API_KEY` (format `keyId:secret`) for
real generative clips.
...
construction, fidelity-first prompting, Higgsfield generation, and FFmpeg render, with a storyboard
review screen, resumable runs that never re-pay for finished clips, and in-app preview and download.
Planned next: further fidelity levers on the generative path, video upscaling, beat-aware pacing with
music, and Tauri desktop packaging.
