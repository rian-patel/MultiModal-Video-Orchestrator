...
- [ ] **Phase 9** — remaining Phase-8 backlog, each currently blocked by an external prerequisite: `upscale_video` (needs a funded cloud.higgsfield.ai key — `.env` key is present but empty), beat-aware pacing + music (no licensed music assets yet), Tauri desktop packaging (needs the Rust toolchain installed).
